...
そのためこの時点で camelk-test0{,1} Namespace に AMQ Streams Operator がインストールされます。
...
以下は <<AMQ Streams Operator>> のセクションで利用する ApplicationSet CR (applicationsets/amqs/applicationset.yaml) に定義するフィールドで検証で用いた例です。
主要な差分だけを対象とします。
Kafak Cluster と Zookeeper の Pod Replica をデフォルトの 3 から 1 に変更する場合、以下のフィールドを定義します。
