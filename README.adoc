...
----
NOTE: 以下のサンプルは先程作成した 2 つの Namespace (camelk-test0{0,1}) に Camel K Operator をインストールします。
+
...
----
NOTE: 主に namespace を修正します、設定例は以下のサンプルを参照下さい。
+
...
以下は git diff の結果です、修正する箇所は # を参照下さい。
+
----
...
+          - path: tenant-projects/camelk/my-team/values.yaml # 新しい values.yaml を配置するディレクトリ名に変更する
...
@@ -33,9 +33,9 @@ spec:
           ...
           parameters:
           - name: "operators.camelk-operator.nexus.username"
-            value: <USERNAME>
...
+            value: nexus # Nexus Repository Manager のパスワードを指定する
           - name: "operators.camelk-operator.nexus.mirror_url"
-            value: <NEXUS_URL>
+            http://nexusrepo-sonatype-nexus-service.dev-tool.svc.cluster.local:8081/repository/maven-public/ # Maven Repository (Nexus Repository Manager) の Kubernetes Service 経由の URL を指定する
       syncPolicy:
         automated:
           prune: false
----
