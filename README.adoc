...
* GitOps Operator の Service Account に Kafka CR を edit する権限を付与する
----
...
----
NOTE: IntegrationPlatform CR と同じ背景で https://github.com/hashnao/gitops-practice/blob/main/clusters/helper-operator/templates/jobs/kafka.yaml[kafka.yaml] Kafka CR に設定を加えるため、 Cluster Role (kafkas.kafka.strimzi.io-v1beta2-edit) を GitOps Operator の Service Account に付与する必要があります。
...
基本的な流れは <<Camel K Operator>> と同様ですが、Operator のインストールに加えて以下のタスクを実施します。

* clusters/helper-operator/templates/jobs/kafka.yaml
  - Kafka Cluster をデプロイする Kafak CR を作成する
  - Kafaka Exporter でメトリクスを公開する ConfigMap を作成する
* clusters/helper-operator/templates/jobs/strimzi-pod-monitor.yaml
  - Prometheus でメトリクスを収集するための PodMonitor CR を作成する
...
----
NOTE: サンプルの tenant-projects/amqs/sample/values.yaml を Main ブランチに配置しているため ApplicationSet CR を作成すると Application CR が生成され、AMQ Streams Operator をインストールする Helm Charts が実行されます。
そのためこの時点で amqs-test0{,1} Namespace に AMQ Streams Operator がインストールされます。
+
