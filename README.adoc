...
* <<AMQ Streams Operator>> のセクションで呼び出す Helm Charts (clusters/helper-operator) は 以下のタスクを実施します。
  ...
  - https://debezium.io/documentation/reference/stable/connectors/oracle.html[Debezium Connector for Oracle] をデプロイするための ImageStream, KafakConnect CR, ConfigMap を作成する https://github.com/hashnao/gitops-practice/blob/amqs/clusters/helper-operator/templates/jobs/kafkaconnect.yaml[clusters/helper-operator/templates/jobs/kafkaconnect.yaml]
  - Debezium が利用する Kakfa Connector を管理するための Kafakconnector CR を作成する https://github.com/hashnao/gitops-practice/blob/amqs/clusters/helper-operator/templates/jobs/kafkaconnector.yaml[clusters/helper-operator/templates/jobs/kafkaconnector.yaml]
...
+
----
oc -n <NAMESPACE> create secret generic <SECRET_NAME> --from-literal=password=<Debeziumが使用するユーザーのパスワード値>
----
NOTE: Debezium がシステム用ユーザーのパスワードを参照できるようにパスワードを定義したシークレットを AMQ Streams Operator をインストールする Namespace に 作成します。
oc create コマンドで Secret を作成する際に引数に指定する `<SECRET_NAME>` には https://github.com/hashnao/gitops-practice/blob/main/clusters/helper-operator/templates/jobs/kafkaconnect.yaml[KafkaConnect CR] の `spec.config.authentication.passwordSecret.secretName` フィールドの `{{ $value.kafkaconnect.name }}` と合致させます。
デフォルトで https://github.com/hashnao/gitops-practice/blob/main/tenant-projects/amqs/sample/values.yaml[values.yaml] の `operators.amqs-operator0.kafkaconnect フィールドの `name: user-information-connec` と合致させるため、<SECRET_NAME> に user-information-connect を指定します。
+
.実行例
----
oc -n camelk-test00 create secret generic user-information-secret --from-literal=password=debezium
----

NOTE:
KafakUser CR を作成するとデフォルトで自動生成されたパスワードを含む Secret が作成されます。
...
以下は <<AMQ Streams Operator>> のセクションで利用する ApplicationSet CR (applicationsets/amqs/applicationset.yaml) に定義するフィールドの設定例です。
Kafak Cluster と Zookeeper の Pod Replica はデフォルトで 3 であるため、GitOps の動作検証など最小構成で検証する際に Pod Replica を 1 に変更する場合、以下のフィールドを定義します。
